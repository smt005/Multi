...
bool TextDrawManager::initText(const string &text, int fontSize, int dpi, int x, int y, unsigned int& textureID, int& width, int& height)
{
...
int TextDrawManager::getTexture(unsigned char* imageFont, int width, int height)
{
    GLuint texture;
...
void TextDrawContainer::draw()
{
    ...
    const float* mat = CameraGLM::current().matPV();
